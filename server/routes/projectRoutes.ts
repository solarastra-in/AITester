...
import { generateBrowserTestDrafts } from '../browserTestGenerator.js';
import { runBrowserTestCase } from '../browserRunner.js';
...
// 6b. Deep-crawl the project's site (or a given URL) and generate real
// browser-based test cases from the pages actually found — page-health
// checks plus form-fill-and-submit tests, exactly as generated by
// suites/generate for HTTP tests, but for real browser automation.
projectRouter.post('/:id/suites/generate-browser-tests', loadProject, async (req: ProjectRequest, res: Response) => {
  const { name, targetUrl, maxPages, maxDepth } = req.body;
  ...
  try {
    const crawl = await crawlSite(effectiveUrl, {
      maxPages: typeof maxPages === 'number' ? maxPages : undefined,
      maxDepth: typeof maxDepth === 'number' ? maxDepth : undefined,
    });

    if (crawl.pages.length === 0) {
      return res.status(502).json({ error: 'Could not reach or parse any pages at that URL.' });
    }

    const drafts = generateBrowserTestDrafts(crawl);
    if (drafts.length === 0) {
      return res.status(502).json({ error: 'No testable pages, forms, or links were found to generate browser tests from.' });
    }
    ...
    const newSuite: Suite = {
      id: suiteId,
      projectId: req.project!.id,
      name: name ? name.trim() : `Browser Test Suite (${new Date().toLocaleDateString()})`,
      source: 'ai_generated',
      createdAt: new Date().toISOString(),
    };
    db.data.suites.push(newSuite);
    ...
    res.status(500).json({ error: err.message || 'Site crawl / browser test generation failed.' });
  }
});
...
projectRouter.post('/:id/cases/:caseId/run', loadProject, async (req: ProjectRequest, res: Response) => {
  ...
  const runRecord: TestRun = {
    ...
    stats: result.stats,
    browserSteps: result.browserSteps,
